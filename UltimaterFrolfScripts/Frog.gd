...
func shootFrog():
	...
	var shootPosition = position + Vector2(rng.randf_range((-(100-accuracy)*1.5), (100-accuracy)*1.5),rng.randf_range((-(100-accuracy)*1.5), (100-accuracy)*1.5))
	var ogDistance = global_position.distance_to(shootPosition)
	var direction = global_position.direction_to(shootPosition)
	var initVelocity = direction * 7
	...
	if (surface[0] == "Water"):
		var area = $Area2D.get_overlapping_bodies()[0]
		var navMesh = area.get_parent().get_parent().get_node("FrolfCourse").get_node("PortalSpawn")
		var spawnPoint = navMesh.get_closest_point(global_position)
		Global.players[str(player)]["frogPosX"] = spawnPoint.x
		Global.players[str(player)]["frogPosY"] = spawnPoint.y
		yield(get_tree().create_timer(0.3), "timeout")
		Global.emit_signal("nextPlayer",player)
		queue_free()
	if (surface[1] == "Hole"):
		Global.players[str(player)]["score"] = Global.players[str(player)]["score"] + Global.frogInScore
		Global.emit_signal("showScore",Global.frogInScore)
